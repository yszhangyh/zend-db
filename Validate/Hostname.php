        ...
        if ((count($domainParts) > 1) && (strlen($value) >= 4) && (strlen($value) <= 254)) {
            ...
            do {
                ...
                if (preg_match('/([^.]{2,10})$/i', end($domainParts), $matches) ||
                    ...
                    if ($this->_validateIdn &&  isset($this->_validIdns[mb_strtoupper($this->_tld)])) {
                    ...
                    foreach ($domainParts as $domainPart) {
                        ...
                        foreach($regexChars as $regexKey => $regexChar) {
                            ...
                            if ($status === false) {
                                Util::iconv_set_encoding($origenc);
                                require_once 'Zend/Validate/Exception.php';
                                throw new Zend_Validate_Exception('Internal error: DNS validation failed');
                            } elseif ($status !== 0) {
                                $length = 63;
                                if (array_key_exists(mb_strtoupper($this->_tld), $this->_idnLength)
                                    && (array_key_exists($regexKey, $this->_idnLength[mb_strtoupper($this->_tld)]))) {
                                    $length = $this->_idnLength[mb_strtoupper($this->_tld)];
                                }
